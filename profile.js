...
function addSampleData() {
    ...
    const penjemputan = storage.getPenjemputan();
    
    // Tambah sample data jika kosong
    if (transactions.length === 0) {
        storage.addTransaction({
            jenis: 'tukar_sampah',
            deskripsi: 'Tukar 2 kg Plastik (PET, HDPE, PVC, LDPE, PP)',
            jumlah: 20000,
            status: 'berhasil',
            detail: { jenis: 'plastik', berat: 2, hargaPerKg: 10000 }
        });
    }
    
    if (penjemputan.length === 0) {
        storage.addPenjemputan({
            tanggalRequest: '2024-01-15',
            jenisSampah: 'plastik',
            alamat: 'Jl. Raya Mojokerto No. 123',
            status: 'menunggu'
        });
    }
}
...
const hargaSampah = {
    ...
    'kaca': 5000, 'ewaste': 32000, 'organik': 1670
};
...
const deskripsiSampah = {
    ...
    'organik': 'Organik (Sisa Makanan, Sampah Kebun)'
};
